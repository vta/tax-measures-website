  ...
  if (filters.grantee) {
    ...
    if (filteredGranteeIds) {
      results.items = results.items.filter(item => {
        ...
        return item.fields.Grantees.some(id => filteredGranteeIds.includes(id))
      })
    }
  }
...
export async function preprocessData(data) {
  ...
  for (const allocation of data.allocations) {
    ...
    if (allocation.fields.Projects) {
      allocation.fields.Grantees = [...allocation.fields.Projects.reduce((memo, projectId) => {
        const project = getProjectByProjectId(projectId, data.projects)
  ...
  for (const payment of data.payments) {
    const project = getProjectByProjectId(payment.fields.Project[0], data.projects)
...
export function getProjectByProjectId(projectId, projects) {
