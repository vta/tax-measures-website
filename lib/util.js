...
import { compact, max, maxBy, sortBy, uniq } from 'lodash';
...
export function updateUrlWithFilters(filters, currentProjectIds) {
  ...
  if (filters) {
    const { grantee, project, category } = filters;
...
export async function applyFilters(filters, projects, categories, grantees) {
  const results = {
    projects: projects,
    filters,
  };
  ...
  if (filters.grantee) {
    ...
    if (filteredGranteeIds) {
      results.projects = results.projects.filter((project) => {
        ...
        return filteredGranteeIds.includes(project.fields.Grantee[0]);
      });
    }
  }
  ...
  if (filters.project) {
    ...
    if (filteredProjectIds) {
      results.projects = results.projects.filter((project) => {
        return filteredProjectIds.includes(project.id);
      });
    }
  }

  results.projects = sortBy(results.projects, [
    sortByFiscalYear,
    sortByGrantee,
  ]);
...
export function preprocessData(data) {
  ...
  for (const project of data.projects) {
    // Populate category and parent category names for each project
    ...
    if (project.fields.Allocations) {
      ...
      if (project.fields.Awards) {
        ...
        project.fields.AuditedExpenditures = uniq(
          project.fields.Awards.reduce((memo, awardId) => {
            const award = getAwardById(awardId, data.awards);

            if (
              award &&
              award.fields['Audited Expenditures'] &&
              award.fields['Audited Expenditures'].length > 0
            ) {
              memo.push(...award.fields['Audited Expenditures']);
            }
  ...
  for (const auditedExpenditure of data.auditedExpenditures) {
    addProjectData(auditedExpenditure, data);
  }
  ...
  for (const project of data.projects) {
    ...
    // Get total audited expenditures for each project
    project.fields.totalAuditedExpenditureAmount = sumCurrency(
      data.auditedExpenditures
        .filter((auditedExpenditure) => {
          if (!auditedExpenditure.fields.Project) {
            return false;
          }

          return auditedExpenditure.fields.Project[0] === project.id;
        })
        .map((auditedExpenditure) => auditedExpenditure.fields.Amount),
    );
...
export function getExpenditureById(id, expenditures) {
...
export function getSingleCategoryCard(filters, categories) {
  ...
  return categoryCards.find((c) => c.key === currentSingleCategory.fields.Name);
}
...
/**
 * Gets the last full month's date based on a given date.
 * If the input date is the last day of its month, returns the same date.
 * Otherwise, returns the last day of the previous month.
 *
 * @param {string|Date|moment.Moment} date - The reference date
 * @returns {moment.Moment} The last full month's date
 *
 * @example
 * // If last modified is August 18, 2025, returns July 31, 2025
 * getLastFullMonthDate('2025-08-18') // Returns moment for July 31, 2025
 *
 * // If last modified is August 31, 2025, returns August 31, 2025
 * getLastFullMonthDate('2025-08-31') // Returns moment for August 31, 2025
 */
export function getLastFullMonthDate(date) {
  const momentDate = moment(date);

  // Check if the date is the last day of its month
  const isLastDayOfMonth = momentDate.date() === momentDate.daysInMonth();

  if (isLastDayOfMonth) {
    // If it's the last day of the month, return the same date
    return momentDate;
  } else {
    // Otherwise, return the last day of the previous month
    return momentDate.subtract(1, 'month').endOf('month');
  }
}

export function getLastModified(items) {
  const modifiedDates = compact(
    items.map((item) => item.fields['Last Modified']),
  ).map((date) => new Date(date));
  return modifiedDates.length ? max(modifiedDates) : null;
}
