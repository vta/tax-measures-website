...
export function getFiltersFromQuery(query) {
  ...
  if (
    initialFilters.grantee !== '' &&
    typeof initialFilters.grantee === 'string'
  ) {
  ...
  if (
    ...
    typeof initialFilters.category === 'string'
  ) {
...
export async function applyFilters(
  ...
  if (filters.category) {
    const filteredCategoryIds = categories
      .filter((c) => filters.category.includes(c.fields.Name))
      .map((c) => c.id);
    if (filteredCategoryIds) {
      results.items = results.items.filter((item) => {
        ...
        return (
          filteredCategoryIds.includes(item.fields.Category.id) ||
          filteredCategoryIds.includes(item.fields['Parent Category'].id)
        );
      });
    }
  }
  ...
  const filteredProjects = projectIds.reduce((memo, projectId) => {
    ...
    if (
      filters.category &&
      !filters.category.includes(project.fields.Category.fields.Name) &&
      !filters.category.includes(project.fields['Parent Category'].fields.Name)
    ) {
      projectMatchesFilters = false;
    }
...
export function preprocessData(data) {
  ...
  for (const project of data.projects) {
    ...
    project.fields.Subcategory =
      !project.fields['Parent Category'].id ||
      project.fields.Category.id === project.fields['Parent Category'].id
        ? { fields: { Name: '' } }
        : cloneDeep(project.fields.Category);
