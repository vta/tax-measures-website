...
export async function applyFilters(filters, awards, expenditures, projects, categories, grantees) {
  const results = {
    items: [],
    projects: [],
    filters
  ...
  const filteredProjects = projectIds.reduce((memo, projectId) => {
    ...
    if (filters.category && !filters.category.includes(project.fields.Category.fields.Name) && !filters.category.includes(project.fields['Parent Category'].fields.Name)) {
      projectMatchesFilters = false
    }

    if (filters.grantee && !filters.grantee.includes(project.fields['Grantee Name'])) {
      projectMatchesFilters = false
    }
...
function addProjectData(item, data) {
  ...
  if (item.fields.Project) {
    ...
    if (project && // Get a list of all grantees for each item
...
export async function preprocessData(data) {
  ...
  for (const project of data.projects) {
    ...
    if (project.fields.Allocations) {
      ...
      if (project.fields.Awards) {
        project.fields.Expenditures = uniq(project.fields.Awards.reduce((memo, awardId) => {
          const award = getAwardById(awardId, data.awards)

          if (award && award.fields.Expenditures && award.fields.Expenditures.length > 0) {
    ...
    project.fields.Subcategory = !project.fields['Parent Category'].id || project.fields.Category.id === project.fields['Parent Category'].id ? { fields: { Name: '' } } : cloneDeep(project.fields.Category)
  ...
  for (const project of data.projects) {
    ...
    project.fields.totalExpenditureAmount = sumCurrency(data.expenditures.filter(expenditure => {
...
export function findLatestDate(dates) {
  return moment.max(dates.filter(date => date !== undefined).map(date => moment(date, 'YYYY-MM-DD')))
}
