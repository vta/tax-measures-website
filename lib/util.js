...
export function updateUrlWithFilters(filters, currentProjectIds) {
  ...
  if (filters) {
    ...
    urlParams.transactionType = transactionType
    urlParams.grantee = grantee
    urlParams.project = project
    urlParams.category = category
  }

  if (currentProjectIds && currentProjectIds.length > 0) {
    urlParams.project_ids = currentProjectIds.join(',')
  }

  window.history.replaceState({}, '', `?${querystring.stringify(urlParams)}`)
}
