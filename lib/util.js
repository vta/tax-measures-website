...
export function applyFilters(filters, awards, payments, projects, categories, grantees) {
  ...
  const projectIds = uniq(results.items.filter(item => !!item.fields.Project).map(item => item.fields.Project[0]))
...
function addProjectData(item, data) {
  ...
  if (item.fields.Allocation) {
    ...
    if (award && award.fields.Project && award.fields.Project.length) {
