...
export async function fetchGeoJson(items) {
  return Promise.all(items.filter(item => item.fields.geojson).map(async item => {
    try {
      ...
      if (response.ok) {
        ...
        item.fields.geometry = geojson
        item.fields.bbox = layerBbox
        item.fields.hasProjectGeometry = true
      }
    } catch (error) {
      console.warn(`Invalid geometry for item "${item.fields.Name}"`)
      console.warn(item.fields.geojson[0])
      console.warn(error)
    }
  }))
}
