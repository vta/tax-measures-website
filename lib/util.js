...
export function formatAvailability(item) {
  if (!item.fields['Available Start'] && !item.fields['Available End']) {
    return ''
  } else if (!item.fields['Available End']) {
    return `starting ${item.fields['Available Start']}`
  } else if (!item.fields['Available Start']) {
    return `through ${item.fields['Available End']}`
  } else {
    return `${item.fields['Available Start']} - ${item.fields['Available End']}`
  }
}
