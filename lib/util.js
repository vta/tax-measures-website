...
function addProjectData(item, data) {
  ...
  if (item.fields.Allocation) {
    const allocation = getAllocationById(item.fields.Allocation[0], data.allocations)

    if (allocation && allocation.fields.Projects && allocation.fields.Projects.length) {
      projectId = allocation.fields.Projects[0]
    }
    ...
    if (award && award.fields.Allocation && award.fields.Allocation.length) {
      const allocation = getAllocationById(award.fields.Allocation[0], data.allocations)
