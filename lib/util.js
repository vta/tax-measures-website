...
export function applyFilters(filters, awards, payments, projects, categories, grantees) {
  ...
  if (filters.startDate) {
    results.items = results.items.filter(item => {
      ...
      } else if (results.transactionType === 'award') {
        itemDate = item.fields['Available End']
      }
  ...
  if (filters.endDate) {
    results.items = results.items.filter(item => {
      ...
      if (results.transactionType === 'payment') {
        ...
        itemDate = item.fields['Available Start']
      }
      ...
      return moment(filters.endDate).startOf('day').isSameOrAfter(item.fields.Date)
    })
  }
...
export async function preprocessData(data) {
  ...
  for (const award of data.awards) {
    const project = getProjectById(award.fields.Project[0], data.projects)

    if (!project) {
      award.fields.category = { fields: { Name: 'Uncategorized' } };
      award.fields['Parent Category'] = { fields: { Name: 'Uncategorized' } };
    } else {
      // Populate category for each award
      ...
      // get a list of all grantees for each award
      if (project.fields.Grantee) {
        award.fields.Grantee = project.fields.Grantee
      }
    }
  }

  for (const payment of data.payments) {
    const project = getProjectById(payment.fields.Project[0], data.projects)

    if (!project) {
      payment.fields.category = { fields: { Name: 'Uncategorized' } };
      payment.fields['Parent Category'] = { fields: { Name: 'Uncategorized' } };
    } else {
      ...
      if (project.fields.Grantee) {
        payment.fields.Grantee = project.fields.Grantee
      }
    }
  }
