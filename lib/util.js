import { cloneDeep, compact, maxBy, pick, sortBy, uniq } from 'lodash'
import fetch from 'isomorphic-unfetch'
import querystring from 'querystring'
import bbox from '@turf/bbox'

const multipleGrantee = {
  id: 'multiple',
  fields: {
    Name: 'Multiple'
  }
...
export function applyFilters(filters, awards, payments, projects, categories, grantees) {
  ...
  if (filters.project) {
    const filteredProjectIds= projects.filter(p => p.fields.Name.toLowerCase().includes(filters.project.toLowerCase())).map(p => p.id)
    if (filteredProjectIds) {
      results.items = results.items.filter(item => {
        if (item.fields.ProjectId) {
          return filteredProjectIds.includes(item.fields.ProjectId)
        } else {
          return false
        }
      })
  ...
  const projectIds = compact(uniq(results.items.map(item => item.fields.ProjectId)))
...
function addProjectData(item, data) {
  ...
  if (item.fields.Allocation) {
    allocation = getAllocationById(item.fields.Allocation[0], data.allocations)

    if (allocation && allocation.fields.Project && allocation.fields.Project.length) {
      projectId = allocation.fields.Project[0]
    }
  } else if (item.fields.Award) {
    const award = getAwardById(item.fields.Award[0], data.awards)

    if (award && award.fields.Allocation && award.fields.Allocation.length) {
      allocation = getAllocationById(award.fields.Allocation[0], data.allocations)
  ...
  item.fields.ProjectId = projectId
...
function createPhantomProject(allocation, data) {
  if (allocation.fields.Project && allocation.fields.Project.length) {
    return
  }

  const awards = data.awards.filter(award => award.fields.Allocation && award.fields.Allocation.includes(award.id))

  const grantees = [...awards.reduce((memo, awardId) => {
    const award = getAwardById(awardId, data.awards)
  
    if (award && award.fields.Grantee && award.fields.Grantee.length) {
      memo.add(award.fields.Grantee[0])
    }
  ...
  const project = {
    id: allocation.id,
    fields: {
      Name: allocation.fields['Allocation ID'],
      Allocations: [allocation.id],
      Grantee: grantees.length === 1 ? grantees : ['multiple'],
      'Fiscal Year': allocation.fields['Available Start'],
    },
    isPhantomProject: true,
  }

  allocation.fields.Project = [project.id]
...
export async function preprocessData(data) {
  ...
  // Add a grantee for "Multiple"
  data.grantees.push(multipleGrantee)

  // Every allocation that doesn't have a project turns into a "Phantom Project"
  for (const allocation of data.allocations) {
    createPhantomProject(allocation, data)
  }
  ...
  for (const project of data.projects) {
    ...
    if (project.fields.Allocations) {
      ...
      project.fields.Awards = uniq(allocations.reduce((memo, allocation) => {
        if (allocation && allocation.fields.Awards && allocation.fields.Awards.length) {
          memo.push(...allocation.fields.Awards)
        }
        return memo
      }, []))
      ...
      project.fields.Payments = uniq(project.fields.Awards.reduce((memo, awardId) => {
        ...
        if (award && award.fields.Payments && award.fields.Payments.length) {
  ...
  for (const project of data.projects) {
    ...
    project.fields.totalPaymentAmount = sumCurrency(data.payments.filter(payment => {
      if (!payment.fields.ProjectId) {
        return false
      }

      return payment.fields.ProjectId === project.id
    ...
    project.fields.totalAllocationAmount = sumCurrency(data.allocations.filter(allocation => {
      if (!allocation.fields.Project || project.isPhantomProject) {
        return false
      }

      return allocation.fields.Project.includes(project.id)
    }).map(allocatiion => allocatiion.fields.Amount))

    // Get total awards for each project  
    project.fields.totalAwardAmount = sumCurrency(data.awards.filter(award => {
      if (!award.fields.ProjectId) {
        return false
      }

      return award.fields.ProjectId === project.id
