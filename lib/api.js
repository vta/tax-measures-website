...
export function fetchTotalExpenditures() {
  return airtableGet('Total Expenditures', {
    'fields[]': ['Expenditure ID', 'Date', 'Amount', 'Award', 'Last Modified'],
    filterByFormula: '{Amount} != BLANK()',
    'sort[0][field]': 'Date',
  });
}

export function fetchAuditedExpenditures() {
  return airtableGet('Audited Expenditures', {
    'fields[]': [
      'Expenditure ID',
      'Date',
      'Amount',
...
export async function fetchFaq() {
  return airtableGet('FAQs', {
    'fields[]': ['Title', 'Content', 'Term'],
    filterByFormula: 'IF(AND({Title} != BLANK(),{Content} != BLANK()), 1, 0)',
    'sort[0][field]': 'Order',
  });
}
