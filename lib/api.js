...
export function fetchAllocations() {
  ...
  return airtableGet('Allocations', {
    ...
    filterByFormula: `IF(AND({Amount} != BLANK(), {Category} != BLANK(), {Available Start} <= ${currentFiscalYear}), 1, 0)`,
...
export function fetchProjects() {
  return airtableGet('Projects', {
    'fields[]': ['Name', 'URL', 'Fiscal Year', 'geojson', 'Longitude', 'Latitude', 'Grantee', 'Allocations', 'Awards', 'Notes', 'Documents', 'Last Modified'],
