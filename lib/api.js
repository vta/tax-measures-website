import { airtableGet } from './airtable.js'
import { vtaGet } from './vta.js'
...
export function fetchAllocations() {
  return airtableGet('Allocations', {
    ...
    filterByFormula: 'IF(OR({Amount} = BLANK(), {Category} = BLANK()), 0, 1)',
...
export function fetchAwards() {
  return airtableGet('Awards', {
    'fields[]': ['Date', 'Award Amount', 'Project', 'Allocation', 'Grantee', 'Expenditures', 'Documents'],
...
export function fetchExpenditures() {
  return airtableGet('Expenditures', {
    'fields[]': ['Date', 'Amount', 'Expenditure Description', 'Award'],
